...
export function validateSubdomain(subdomain) {
	if (!subdomain) {
		return __('Subdomain cannot be empty');
	}
	if (subdomain.length < 5) {
		return __('Subdomain too short. Use 5 or more characters');
	}
	if (subdomain.length > 32) {
		return __('Subdomain too long. Use 32 or less characters');
	}
	if (!subdomain.match(/^[a-z0-9][a-z0-9-]*[a-z0-9]$/)) {
		return __('Subdomain contains invalid characters. Use lowercase characters, numbers and hyphens');
	}
	return null;
}
