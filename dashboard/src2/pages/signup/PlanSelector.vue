        ...
        <div class="relative col-span-1 flex h-full w-full items-center justify-center md:bg-white overflow-hidden">
            ...
            >
                ...
                <div class="mt-6 w-full overflow-hidden">
                    <div v-if="plans && plans.length" class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-3 lg:gap-4 xl:gap-6 2xl:gap-8 max-h-[60vh] md:max-h-none overflow-y-auto md:overflow-visible">
                            ...
                            <div class="flex-grow overflow-hidden">
                                <div class="mb-3 lg:mb-4 xl:mb-5 border-b border-gray-100 pb-3 lg:pb-4 xl:pb-5">
                                    <div class="flex items-start justify-between mb-2 lg:mb-3">
                                        <h3 class="text-lg sm:text-xl md:text-base lg:text-lg xl:text-xl 2xl:text-2xl font-bold text-gray-900 leading-tight line-clamp-2 break-words">
                                            {{ currentLang === 'vi' ? planItem.title : planItem.title_en }}
                                        </h3>
                                        <span v-if="planItem.popular" class="ml-2 px-2 py-1 lg:px-3 lg:py-1.5 xl:px-4 xl:py-2 text-xs lg:text-sm font-medium text-white bg-gradient-to-r from-orange-400 to-pink-500 rounded-full flex-shrink-0">
                                            HOT
                                        </span>
                                    </div>
                                    <div class="flex items-center justify-center w-full px-3 py-1.5 lg:px-6 lg:py-3 xl:px-5 xl:py-2.5 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg mb-2">
                                        <span class="text-base sm:text-lg md:text-sm lg:text-base xl:text-sm 2xl:text-xl font-bold text-blue-700 text-center">
                                            {{ planItem.label }}
                                        </span>
                                    </div>
                                    <p class="text-xs sm:text-sm md:text-xs lg:text-sm xl:text-base text-gray-600 leading-relaxed">
                                        {{ planItem.sublabel }}
                                    </p>
                                </div>
                                <ul class="my-2 lg:my-3 xl:my-4 space-y-1 lg:space-y-2 xl:space-y-3">
                                    <li v-for="feature in planItem.features" :key="feature.value" class="flex items-start">
                                        <svg v-if="feature.icon === 'check-circle'" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 md:h-3 md:w-3 lg:h-4 lg:w-4 xl:h-5 xl:w-5 2xl:h-6 2xl:w-6 mt-0.5 mr-1 lg:mr-2 xl:mr-3 text-green-500 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                                        </svg>
                                        <span class="text-sm sm:text-base md:text-xs lg:text-sm xl:text-base 2xl:text-lg leading-tight break-words min-w-0 flex-1">{{ __(feature.value) }}</span>
                                    </li>
                                </ul>
                            </div>
