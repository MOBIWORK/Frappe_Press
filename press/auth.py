...
def hook():
    ...
    user_type = frappe.get_cached_value(
    ...
    for denied in DENIED_WILDCARD_PATHS:
        if path.startswith(denied):
            ...
            log(path, user_type)
            frappe.throw("Access not allowed for this URL",
...
def flush():
    log_file = os.path.join(frappe.utils.get_bench_path(),
                            "logs", "press.auth.json.log")
    try:
