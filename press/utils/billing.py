...
def get_razorpay_client():
    ...
    if not hasattr(frappe.local, "press_razorpay_client_object"):
        key_id = frappe.db.get_single_value(
            "Press Settings", "razorpay_key_id")
        ...
        frappe.local.press_razorpay_client_object = razorpay.Client(
...
def process_micro_debit_test_charge(stripe_event):
    try:
        ...
        log_error("Error Processing Stripe Micro Debit Charge",
                  body=stripe_event)
