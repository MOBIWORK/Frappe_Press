...
def log_error(title, **kwargs):
    ...
    serialized = json.dumps(
        kwargs, indent=4, sort_keys=True, default=str, skipkeys=True)
    message = f"Data:\n{serialized}\nException:\n{traceback}"
...
class RemoteFrappeSite:
    ...
    def _validate_missing_backups(self):
        ...
        if missing_files:
            missing_config = "site config and " if not self.backup_links.get(
                "config") else ""
            missing_backups = (
    ...
    def _processed_backups_from_response(self):
        for file_type, file_path in self._fetch_latest_backups_response.items():
            self._remote_backup_links[file_type] = self.__process_frappe_url(
                file_path)
    ...
    return list(
        set(
            [
                ...
                + frappe.get_all("Site Config Key",
                                 fields=["`key`"], filters={"internal": True})
            ]
        )
    )
...
def developer_mode_only():
    if not frappe.conf.developer_mode:
        frappe.throw("You don't know what you're doing. Go away!",
