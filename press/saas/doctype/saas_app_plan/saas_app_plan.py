...
class SaasAppPlan(Document):
    ...
    def get_total_amount(self, payment_option):
        ...
        amount = frappe.db.get_value(
            "Plan", self.plan, f"price_{team.currency.lower()}")
    ...
    def validate_payout_percentage(self):
        ...
        site_plan = frappe.db.get_value("Plan", self.site_plan, "price_vnd")
        saas_plan = frappe.db.get_value("Plan", self.plan, "price_vnd")
        self.payout_percentage = 100 - \
            float("{:.2f}".format((site_plan / saas_plan) * 100))
