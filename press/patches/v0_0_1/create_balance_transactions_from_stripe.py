...
def execute():
    ...
    teams = frappe.db.get_all(
        "Team",
        filters={"name": ("not in", skip_teams),
...
def create_balance_transactions_for_team(name):
    ...
    if frappe.db.exists(
            "Balance Transaction", {
                "source": "Free Credits", "team": team.name}
    ):
    ...
    for transaction in transactions:
        ...
        if type == "Applied To Invoice":
            invoice = frappe.get_doc(
                "Invoice", {"stripe_invoice_id": transaction.invoice})
            invoice_name = invoice.name
            free_credits_applied = apply_to_invoice(
                invoice, amount, free_credits_left, balance_transactions
            )
            free_credits_left = frappe.utils.rounded(
                free_credits_left - free_credits_applied, 2)
    ...
    team_balance = team.get_balance_all()
...
def apply_to_invoice(invoice, amount, free_credits_left, balance_transactions):
    unallocated_bts = get_last_unallocated_balance_transactions(
        balance_transactions)
    ...
    for bt in unallocated_bts:
        ...
        row = bt.append(
            "allocated_to",
            {"invoice": invoice.name, "amount": to_apply,
             "currency": invoice.currency},
        )
        bt.save()
        bt.reload()
