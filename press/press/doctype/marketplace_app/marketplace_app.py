...
class MarketplaceApp(WebsiteGenerator):
            ...
            source_doc = frappe.get_doc(
                {
                    ...
                    "repository_url": frappe.db.get_value(
                        "App Source", {
                            "name": self.sources[0].source}, "repository_url"
                    ),
                    "public": 1,
                }
            )
    ...
    def remove_version(self, version):
        if self.status == "Published" and len(self.sources) == 1:
            frappe.throw(
                "Failed to remove. Need at least 1 version for a published app")
    ...
    def fetch_readme(self):
        ...
        if source.github_installation_id:
            ...
            github_access_token = frappe.get_value(
                "Press Settings", None, "github_access_token")
    ...
    def get_context(self, context):
        ...
        for source in self.sources:
            ...
            frappe_source_name = frappe.get_doc(
                "Release Group App", {"app": "frappe",
                                      "parent": unique_public_rgs[source.version]}
            ).source
        ...
        developer_review_reply = frappe.qb.DocType("Developer Review Reply")
        user = frappe.qb.DocType("User")

        for review in user_reviews:
            query = (
                frappe.qb.from_(developer_review_reply)
                .join(user)
                .on(user.name == developer_review_reply.developer)
                .select(
                    developer_review_reply.description,
                    user.full_name.as_("user_name"),
                    user.user_image,
                    developer_review_reply.creation
                )
                .where(developer_review_reply.review == review.name)
                .orderby(developer_review_reply.creation, order=Order.desc)
            )
        ...
        for source in self.sources:
            ...
            for rg_name in release_groups:
                ...
                if latest_active_bench:
                    sources_on_bench = [
                        a.source for a in latest_active_bench.apps]
    ...
    def get_payout_amount(self, status: str = "", total_for: str = "net_amount"):
        ...
        payout_orders = frappe.get_all(
            "Payout Order", filters=filters, pluck="name")
    ...
    def should_create_usage_record(self, marketplace_app_plan, plan, site):
        ...
        is_free = frappe.db.get_value(
            "Marketplace App Plan", marketplace_app_plan, "is_free")
...
def marketplace_app_hook(app=None, site="", op="install"):
    if app is None:
        site_apps = frappe.get_all(
            "Site App", filters={"parent": site}, pluck="app")
