...
class PayoutOrder(Document):
        ...
        for row in self.items:
            ...
            app_payment = (
                frappe.get_cached_doc(
                    "Marketplace App Payment", row.document_name)
            ...
            row.commission = (
                app_payment.get_commission(
                    row.total_amount) if not self.ignore_commission else 0.0
            )

            row.net_amount = row.total_amount - row.commission

            if row.currency == "VND":
                app_payment.total_vnd += row.net_amount if row.net_amount > 0 else row.commission
    ...
    def validate_net_totals(self):
        ...
        for row in self.items:
            if row.currency == "VND":
                self.net_total_vnd += row.net_amount
            elif row.currency == "INR":
    ...
    for app_team, items in groupby(items, key=lambda x: x["app_team"]):
        try:
            ...
            po_exists = frappe.db.exists(
                "Payout Order", {"recipient": app_team,
            ...
            frappe.db.rollback()
            log_error("Payout Order Creation Error",
                      team=app_team, invoice_items=items)
...
def get_unaccounted_marketplace_invoice_items():
    ...
    items = (
        ...
        .select(
            invoice_item.name, invoice_item.document_name, marketplace_app.team.as_(
                "app_team")
        )
        .run(as_dict=True)
    )
