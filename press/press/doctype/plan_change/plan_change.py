...
class PlanChange(Document):
    def validate(self):
        self.team = frappe.db.get_value(
            self.document_type, self.document_name, "team")
        if self.from_plan and not self.type:
            from_plan_value = frappe.db.get_value(
                "Plan", self.from_plan, "price_vnd")
            to_plan_value = frappe.db.get_value(
                "Plan", self.to_plan, "price_vnd")
    ...
    def change_subscription_plan(self):
        ...
        if not subscription:
            frappe.throw(
                f"No subscription for {self.document_type} {self.document_name}")
