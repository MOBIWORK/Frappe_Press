...
class AppReleaseApprovalRequest(Document):
    def before_save(self):
        apps = frappe.get_all(
            "Featured App", {"parent": "Marketplace Settings"}, pluck="app")
    ...
    def request_already_exists(self):
        ...
        if len(requests) > 0:
            frappe.throw(
                "An active request for this app release already exists!")

    def another_request_awaiting_approval(self):
        request_source = frappe.db.get_value(
            "App Release", self.app_release, "source")

        releases_awaiting_approval = frappe.get_all(
            "App Release Approval Request",
            filters={"marketplace_app": self.marketplace_app,
                     "status": "Open"},
            pluck="app_release",
        )
    ...
    def publish_status_change(self, source):
        frappe.publish_realtime(
            event="request_status_changed", message={"source": source})
    ...
    def start_screening(self):
        self.release = frappe.get_doc(
            "App Release", self.app_release, for_update=True)
        self._set_baseline()
    ...
    def _set_baseline(self):
        approved_releases = frappe.get_all(
            ...
            filters={"status": "Approved", "app": self.app,
                     "name": ("!=", self.name)},
    ...
    def _screen_python_files(self):
        files = glob.glob(self.release.clone_directory +
                          "/**/*.py", recursive=True)
        result = []
        for file in files:
        ...
        for index, line in enumerate(lines):
            ...
            for severity, violations in configuration.items():
                for violation, keywords in violations.items():
                    ...
                    if search:
                        issues.append(
                            {"severity": severity, "violation": violation,
    ...
    def _render_html(self):
        ...
        for file in diff_result:
            ...
            for line in file["lines"]:
                line["highlighted_context"] = highlight_context(
                    line["context"])
