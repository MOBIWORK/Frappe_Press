...
class DripEmail(Document):
    ...
    def send_drip_email(self, site_name):
        ...
        account_request = frappe.get_doc(
            "Account Request", site.account_request)
    ...
    def select_consultant(self, site) -> str:
        ...
        if not site.erpnext_consultant:
            ...
            site.erpnext_consultant = get_random(
                "ERPNext Consultant", dict(active=1))
            frappe.db.set_value("Site", site.name,
                                "erpnext_consultant", site.erpnext_consultant)

        consultant = frappe.get_doc(
            "ERPNext Consultant", site.erpnext_consultant)
...
def send_drip_emails():
    ...
    drip_emails = frappe.db.get_all(
        "Drip Email", {"enabled": 1, "email_type": (
            "in", ("Drip", "Onboarding"))}
    )
