...
def get_new_site_options():
    ...
    if site_options_sc.ok:
        ...
        print("Couldn't retrive New site information: {}".format(
            site_options_sc.status_code))
...
def upload_backup_file(file_type, file_name, file_path):
    def _update_progress_bar(monitor):
        update_progress_bar(
            "Uploading {} file".format(
                file_type), monitor.bytes_read, monitor.len
        )
    ...
    upload_ticket = session.get(remote_link_url, data={
                                "file": file_name, "parts": parts})
    ...
    if parts > 1:
        ...
        for count in range(parts):
            ...
            update_progress_bar(
                "Uploading {} File".format(file_type), count, parts)
...
def render_plan_table(plans_list):
    ...
    for plan in plans_list:
        ...
        plans_table.append(
            [plan, "{} hour{}/day".format(cpu_time,
                                          "" if cpu_time < 2 else "s")]
        )
...
def handle_request_failure(request=None, message=None, traceback=True, exit_code=1):
    message = message or "Request failed with error code {}".format(
        request.status_code)
...
def select_primary_action():
    ...
    idx = click.prompt("What do you want to do?",
                       type=click.IntRange(1, len(actions))) - 1
...
def select_site():
    ...
    if get_all_sites_request.ok:
        ...
        sites_info = {site["name"]: get_site_info(
            site["name"]) for site in all_sites}
...
def select_team(session):
    ...
    if len(available_teams) == 1:
        ...
        idx = click.prompt("Select Team", type=click.IntRange(
            1, len(available_teams))) - 1
...
def is_valid_subdomain(subdomain):
    if len(subdomain) < 5:
        print("Tên miền quá ngắn. Hãy sử dụng 5 ký tự trở lên.")
        return False
...
def check_app_compat(available_group):
    ...
    for (app, branch) in existing_group:
        info = [(a["app"], a["branch"])
                for a in available_group["apps"] if a["app"] == app]
        if info:
            app_title, available_branch = info[0]

            if branch != available_branch:
                print("⚠️  App {}:{} => {}".format(
                    app, branch, available_branch))
...
def filter_apps(versions):
    ...
    while True:
        ...
        try:
            ...
            selected_group = find(
                selected_version["groups"], lambda g: g["name"] == group)
        except IndexError:
            print("Invalid Selection ❌")
            continue
...
def upload_backup(local_site):
    ...
    for x, (file_type, file_path) in enumerate(
        ...
        if uploaded_file:
            ...
            print(
                "Cannot create site on Frappe Cloud without all site backup files uploaded.")
            print("Exitting...")
            sys.exit(1)
...
def restore_site(local_site):
    ...
    site_restore_request = session.post(
        restore_site_url, payload, headers=headers)

    if site_restore_request.ok:
        print("Your site {0} is being restored on {1} ✨".format(
            local_site, selected_site))
        print(
            "View your site dashboard at https://{}/dashboard/sites/{}".format(
...
def create_session():
    ...
    if login_sc.ok:
        ...
        session.headers.update(
            {"X-Press-Team": team, "Connection": "keep-alive"})
        return session
    else:
...
def frappecloud_migrator(local_site):
    ...
    upload_url = "{}://{}/api/method/press.api.site.new".format(
        scheme, remote_site)
    ...
    site_info_url = "{}://{}/api/method/press.api.site.get".format(
        scheme, remote_site)
    account_details_url = "{}://{}/api/method/press.api.account.get".format(
        scheme, remote_site
    )
    all_site_url = "{}://{}/api/method/press.api.site.all".format(
        scheme, remote_site)
