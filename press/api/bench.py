...
def new(bench, lang='vi'):
    ...
    group = new_release_group(
        ...
        team.name,
        bench["cluster"],
        bench["saas_app"] if frappe.db.exists(
            "Saas App", bench["saas_app"]) else "",
        bench["server"],
    )
    return group.name
...
def get(name):
    ...
    return {
        ...
        "tags": frappe.get_all(
            "Press Tag", {"team": group.team,
                          "doctype_name": "Release Group"}, ["name", "tag"]
        ),
    }
...
def all(server=None, bench_filter=None):
    ...
    if bench_filter["tag"]:
        ...
        query = query.inner_join(press_tag).on(
            (press_tag.tag_name == bench_filter["tag"]) & (
                press_tag.parent == group.name)
        )
    ...
    for group in private_groups:
        group.tags = frappe.get_all(
            "Resource Tag", {"parent": group.name}, pluck="tag_name")
...
def dependencies(name: str):
    ...
    bench_dependencies = frappe.get_all(
        "Bench Dependency", ["name", "title", "internal"])

    return {
        "active_dependencies": active_dependencies,
        "supported_dependencies": list(
            # deduplicate dependencies
            {d["value"]: d for d in supported_dependencies + \
             active_dependencies}.values()
        ),
...
def versions(name):
    ...
    for version in deployed_versions:
        version.sites = find_all(
            sites_in_group_details, lambda x: x.bench == version.name)
        for site in version.sites:
            site.version = rg_version
            site.server_region_info = find(
                cluster_data, lambda x: x.name == site.cluster)
    ...
    for app in apps:
        app.update(
            frappe.db.get_value(
                ...
                app.source,
                ("branch", "repository",
                 "repository_owner", "repository_url"),
                as_dict=1,
                cache=True,
            )
        )
...
def get_branches_for_marketplace_app(
    ...
    for marketplace_app_source in marketplace_app.sources:
        app_source = frappe.get_doc(
            "App Source", marketplace_app_source.source)
