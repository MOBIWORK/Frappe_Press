...
def get_jwt_token():
    key = frappe.db.get_single_value(
        "Press Settings", "github_app_private_key")
    app_id = frappe.db.get_single_value("Press Settings", "github_app_id")
    now = datetime.now()
    expiry = now + timedelta(minutes=9)
    payload = {"iat": int(now.timestamp()), "exp": int(
        expiry.timestamp()), "iss": app_id}
...
def clear_token_and_get_installation_url():
    ...
    public_link = frappe.db.get_single_value(
        "Press Settings", "github_app_public_link")
    return f"{public_link}/installations/new"


def clear_current_team_access_token():
    team = get_current_team()
    frappe.db.set_value("Team", team, "github_access_token",
                        "")  # clear access token
...
@frappe.whitelist()
def app(installation, owner, repository, branch, lang='vi'):
    def _construct_tree(tree, children, children_map):
        for file in children:
            if file.path in children_map:
                tree[file.name] = _construct_tree(
                    {}, children_map[file.path], children_map)
            else:
                tree[file.name] = None
        return tree
    ...
    if any(x in tree for x in py_setup_files):
        for directory, files in tree.items():
            if files and ("hooks.py" in files) and ("patches.txt" in files):
                ...
                reason_for_invalidation = (
                    _("Files {0} does not exist inside {2}/{3} directory.", lang).format(
                        frappe.bold('hooks.py or patches.txt'), directory, directory)
                )
    else:
        reason_for_invalidation = (
            _("Files {0} do not exist in app directory.", lang).format(
                frappe.bold(' or '.join(py_setup_files)))
        )

    if not (app_name and title):
        frappe.throw(_("Not a valid Frappe App! {0}", lang).format(
            reason_for_invalidation))
