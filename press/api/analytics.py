...
def get(name, timezone, duration="7d"):
    ...
    plan_limit = get_plan_config(plan).get(
        "rate_limit", {}).get("limit") if plan else 0
...
def get_uptime(site, timezone, timespan, timegrain):
    monitor_server = frappe.db.get_single_value(
        "Press Settings", "monitor_server")
    if not monitor_server:
        return []

    url = f"https://{monitor_server}/prometheus/api/v1/query_range"
    password = get_decrypted_password(
        "Monitor Server", monitor_server, "grafana_password")
    ...
    response = requests.get(
        url, params=query, auth=("frappe", password)).json()
            ...
            frappe._dict(
                {
                    "date": convert_utc_to_timezone(
                        datetime.fromtimestamp(timestamp).replace(
...
def get_request_by_path(site, query_type, timezone, timespan, timegrain):
    ...
    password = get_decrypted_password(
        "Log Server", log_server, "kibana_password")
                        ...
                        "filter": {
                            "bool": {
                                "filter": [
                                    ...
                                    {"range": {
                                        "@timestamp": {"gte": f"now-{timespan}s", "lte": "now"}}},
                                ],
    ...
    for bucket in response["aggregations"]["date_histogram"]["buckets"]:
        buckets.append(
            frappe._dict(
                {
                    "date": convert_utc_to_timezone(
                        get_datetime(bucket["key_as_string"]).replace(
                            tzinfo=None), timezone
                    ),
...
def get_current_cpu_usage(site):
    try:
        ...
        response = requests.post(
            url, json=query, auth=("frappe", password)).json()
...
def multi_get_current_cpu_usage(sites):
    try:
        ...
        for response in response["responses"]:
            hits = response["hits"]["hits"]
            if hits:
                result.append(hits[0]["_source"]["json"]
                              ["request"].get("counter", 0))
            else:
                result.append(0)
        return result
    except Exception:
    ...
    for d in response["hits"]["hits"]:
        ...
        data["timestamp"] = convert_utc_to_timezone(
            frappe.utils.get_datetime(
                data["timestamp"]).replace(tzinfo=None), timezone
        )
        out.append(data)
...
def binary_logs(name, start_time, end_time, pattern=".*", max_lines=4000):
    ...
    db_server = frappe.db.get_value(
        "Server", site_doc.server, "database_server")
...
def plausible_analytics(name):
    ...
    headers = {
        "Authorization": f'Bearer {settings.get_password("plausible_api_key")}'}

    for api_type, endpoint in api_endpoints.items():
        res = requests.get(settings.plausible_url + endpoint,
                           params=params, headers=headers)
        if res.status_code == 200 and res.json().get("results"):
            res = res.json().get("results")
            if api_type == "aggregate":
                response.update(
                    {"agg_pageviews": res["pageviews"],
                        "agg_visitors": res["visitors"]}
                )
            elif api_type == "timeseries":
                pageviews = [{"value": d["pageviews"],
                              "date": d["date"]} for d in res]
                unique_visitors = [
                    {"value": d["visitors"], "date": d["date"]} for d in res]
                response.update(
                    {"pageviews": pageviews, "visitors": unique_visitors})
