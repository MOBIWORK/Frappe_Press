...
def all(server_filter=None):
    ...
    if server_filter["server_type"] != "Database Servers":
        ...
        if server_filter["tag"]:
            app_server_query = app_server_query.inner_join(res_tag).on(
                (res_tag.parent == app_server.name) & (
                    res_tag.tag_name == server_filter["tag"])
            )
    ...
    for server in servers:
        ...
        server["plan"] = (
            frappe.get_doc(
                "Plan", server_plan_name) if server_plan_name else None
        )
...
def get(name):
    ...
    return {
        ...
        "tags": frappe.get_all(
            "Press Tag", {"team": server.team,
                          "doctype_name": "Server"}, ["name", "tag"]
        ),
    }
...
def overview(name):
    ...
    if server.is_self_hosted:  # Hacky way to show current specs in place of Plans
        ...
        if plan:
            ...
            plan.disk = self_hosted_server.total_storage.split(
                " ")[0]  # Saved in DB as "50 GB"
    return {
...
def usage(name):
    ...
    for usage_type, query in query_map.items():
        response = prometheus_query(
            query[0], query[1], "Asia/Kolkata", 120, 120)["datasets"]
        if response:
    ...
    for timestamp, _ in response["data"]["result"][0]["values"]:
        labels.append(
            convert_utc_to_timezone(
                datetime.fromtimestamp(timestamp, tz=tz.utc).replace(
