...
def approve_partner_request(key):
    partner_request_doc = frappe.get_doc(
        "Partner Approval Request", {"key": key})
    ...
    if partner_request_doc and partner_request_doc.status == "Pending":
        ...
        customer_team = frappe.get_doc(
            "Team", partner_request_doc.requested_by)
...
def request_team_deletion():
    team = get_current_team(get_doc=True)
    doc = frappe.get_doc(
        {"doctype": "Team Deletion Request", "team": team.name}).insert()
    return doc.name
...
def validate_request_key(key, timezone=None):
    ...
    if account_request:
        ...
        possible_country = data.get(
            "country") or get_country_from_timezone(timezone)
        ...
        saas_product_doc = (
            frappe.get_doc(
                "SaaS Product", saas_product) if saas_product else None
        )
...
def get_permissions():
    user = frappe.session.user
    groups = tuple(
        frappe.get_all("Press Permission Group User",
                       {"user": user}, pluck="parent")
        + ["1", "2"]
...
def switch_team(team):
    ...
    if user_is_part_of_team or user_is_system_user:
        frappe.db.set_value("Team", frappe.session.user,
                            "last_used_team", team)
        return {
...
def get_billing_information(timezone=None):
    ...
    if team.billing_address:
        billing_details = frappe.get_doc(
            "Address", team.billing_address).as_dict()
...
def add_partner(referral_code: str):
    ...
    partner = frappe.get_doc(
        "Team", {"partner_referral_code": referral_code}).name
    doc = frappe.get_doc(
        {
...
def update_permissions(user, ptype, updated):
    ...
    for doctype, docs in updated.items():
        for doc, updated_perms in docs.items():
            ...
            values += [(frappe.generate_hash(4), ptype_cap,
                        doctype, doc, user, a) for a in add]
    ...
    if values:
        frappe.db.bulk_insert(
            "Press User Permission",
            fields=["name", "type", "document_type",
                    "document_name", ptype, "action"],
            values=set(values),
...
def groups():
    return frappe.get_all(
        "Press Permission Group", {"team": get_current_team()}, [
            "name", "title"]
    )
...
def add_permission_group(title):
    doc = frappe.get_doc(
        {"doctype": "Press Permission Group",
            "team": get_current_team(), "title": title}
    ).insert(ignore_permissions=True)
