...
def get_candidate_sites(app_name: str) -> List[str]:
    ...
    sites = (
        ...
        # must be installed on bench (corresponding bench app exists)
        .right_join(bench_app)
        .on((bench.name == bench_app.parent) & (bench_app.app == app_name))
        .select(site.name)
        .where(
            (site.status == "Active") & (
                site.team == team) & site_app.app.isnull()
...
def get_app(name: str) -> Dict:
    ...
    for source in app.sources:
        source.source_information = frappe.get_doc(
            "App Source", source.source).as_dict()
...
def options_for_marketplace_app() -> Dict[str, Dict]:
    ...
    for version in versions:
        ...
        version["apps"] = [app for app in version["apps"]
                           if app["name"] != "frappe"]
        ...
        for app in version["apps"]:
            if not is_on_marketplace(app["name"]):
                ...
                marketplace_app = frappe.get_doc(
                    "Marketplace App", app["name"])
                marketplace_versions = [
                    v.version for v in marketplace_app.sources]
...
def add_app(source: str, app: str, lang: str = 'vi'):
    if not is_on_marketplace(app):
        ...
        marketplace_app = frappe.get_doc(
            ...
            sources=[{"version": v, "source": source}
...
def analytics(name: str):
    marketplace_app_doc: MarketplaceApp = frappe.get_doc(
...
def get_marketplace_subscriptions_for_site(site: str):
    ...
    for subscription in subscriptions:
        ...
        subscription.billing_type = is_prepaid_marketplace_app(
            subscription.app)
...
def get_apps_with_plans(apps, release_group: str):
    ...
    frappe_version = frappe.db.get_value(
        "Release Group", release_group, "version")
    for app in m_apps:
        app_source = frappe.db.get_value(
            "Release Group App", {
                "parent": release_group, "app": app.name}, "source"
        )
...
def create_app_plan(marketplace_app: str, plan_data: Dict, lang: str = 'vi'):
    ...
    app_plan_doc = frappe.get_doc(
        {"doctype": "Marketplace App Plan",
            "app": marketplace_app, "plan": plan.name}
    )
...
def get_plan(name):
    plan, gst, discount_percent, block_monthly = frappe.db.get_value(
        "Marketplace App Plan", name, [
            "plan", "gst", "discount_percent", "block_monthly"]
    )
    currency = get_current_team(True).currency.lower()
    title, amount = frappe.db.get_value(
        "Plan", plan, ["plan_title", f"price_{currency}"])
    ...
    if team.erpnext_partner and frappe.get_value(
        ...
        if response.ok:
            ...
            if partner_type is not None:
                discount = partner_discount_percent.get(
                    partner_type) or discount
...
def subscriptions():
    team = get_current_team(True)
    free_plans = frappe.get_all("Marketplace App Plan", {
                                "is_free": 1}, pluck="name")
...
def options_for_version(name, source):
    frappe_version = frappe.get_all(
        "Frappe Version", {"public": True}, pluck="name")
