...
def create_marketplace_usage_record():
    ...
    for subscription in frappe.get_all(
            "Marketplace App Subscription",
            {
                "app": (
                    "in",
                    frappe.get_all("Saas Settings", {
                        "billing_type": "prepaid"}, pluck="name"),
                ),
                "plan": (
                    "not in",
                    frappe.get_all(
                        "Plan", {"document_type": "Marketplace App", "price_vnd": 0}, pluck="name"
                    ),
                ),
                "status": "Active",
            },
        ...
        plan, price = frappe.db.get_value(
            "Plan", subscription["plan"], [
                "plan_title", f"price_{currency.lower()}"]
        )
        ...
        try:
            ...
            marketplace_add_ons = {
                rec["type"]: {
                    ...
                    "price_vnd": rec["price_vnd"] / 31,
                }
                for rec in frappe.get_list(
                    "Marketplace Add On",
                    {"parent": subscription["marketplace_app_plan"]},
                    ["type", "price_usd", "price_inr", "price_vnd"],
                )
            }

            for rec in result.items():
                price = marketplace_add_ons[rec[0]
