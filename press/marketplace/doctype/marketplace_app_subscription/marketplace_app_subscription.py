...
class MarketplaceAppSubscription(Document):
    ...
    def validate_marketplace_app_plan(self):
        app = frappe.db.get_value(
            "Marketplace App Plan", self.marketplace_app_plan, "app")
    ...
    def on_update(self):
        if self.has_value_changed("marketplace_app_plan"):
            ...
            frappe.db.set_value(
                "Subscription", self.subscription, "plan", self.plan)

        if self.has_value_changed("team"):
            frappe.db.set_value(
                "Subscription", self.subscription, "team", self.team)
